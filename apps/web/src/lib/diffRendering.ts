...
/**
 * Shared @pierre/diffs theme bridge for every code surface (diffs and plain files): route the
 * library chrome through the chat code font and the app's surface colors. The styles must be
 * injected as `unsafeCSS` because the library renders inside shadow-rooted markup.
 */
export function buildWorkspaceCodeUnsafeCSS(theme: "light" | "dark"): string {
