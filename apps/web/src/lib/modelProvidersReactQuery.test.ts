...
import {
  modelProvidersQueryKeys,
  modelProvidersQueryOptions,
  saveModelProvidersMutationOptions,
} from "./modelProvidersReactQuery";
...
it("reads configuration from the requested directory with a separate cache", async () => {
  const listModelProviders = vi
    .fn()
    .mockResolvedValue({ path: "/custom/models.json", providers: {} });
  vi.spyOn(nativeApi, "ensureNativeApi").mockReturnValue({
    server: { listModelProviders },
...
it("refreshes active model pickers, invalidates inactive pickers, and keeps directory caches isolated", async () => {
  ...
  await saveModelProvidersMutationOptions(client).onSuccess(saved, {
    agentDir: "/custom",
