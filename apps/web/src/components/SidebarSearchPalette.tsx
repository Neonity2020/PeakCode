...
export type ImportProviderKind = ProviderKind;
...
function buildThemeCommandItem(
  input: {
    query: string;
    resolvedTheme: "light" | "dark";
    theme: "system" | "light" | "dark";
  },
  labels: Messages["searchPalette"],
  ...
  if (hasTokenEqual(normalizedQuery, "system")) {
    return {
      ...
      label: labels.followSystemTheme,
      description: labels.followSystemThemeDescription,
      mode: "system",
  ...
  if (hasTokenEqual(normalizedQuery, "light")) {
    return {
      ...
      label: labels.switchToLightTheme,
      description: labels.lightThemeDescription,
      mode: "light",
  ...
  if (hasTokenEqual(normalizedQuery, "dark")) {
    return {
      ...
      label: labels.switchToDarkTheme,
      description: labels.darkThemeDescription,
      mode: "dark",
  ...
  if (
    ...
    return {
      ...
      label: nextMode === "light" ? labels.switchToLightTheme : labels.switchToDarkTheme,
      description:
        nextMode === "light" ? labels.lightThemeDescription : labels.darkThemeDescription,
      mode: nextMode,
...
function threadMatchLabel(
  ...
  if (input.matchKind === "message") {
    return input.messageMatchCount > 1
      ? labels.chatHits(input.messageMatchCount)
      : labels.chatMatch;
  }
  if (input.matchKind === "project") {
    return labels.projectMatch;
  }
  return null;
}
...
export function SidebarSearchPalette(props: SidebarSearchPaletteProps) {
  ...
  useEffect(() => {
    if (!props.open) {
      ...
      setImportProvider(props.importProviders[0] ?? "pi");
      setImportId("");
      setImportError(null);
  ...
  const themeCommandItem = useMemo(
    () =>
      buildThemeCommandItem(
        ...
        messages.searchPalette,
      ),
    [messages.searchPalette, query, resolvedTheme, theme],
  );
  const currentCodeThemeItems = useMemo<SidebarSearchTheme[]>(
    () =>
      getAvailableCodeThemes(resolvedTheme).map((option) => ({
        ...
        description:
          resolvedTheme === "dark"
            ? messages.searchPalette.applyToDarkSlot
            : messages.searchPalette.applyToLightSlot,
        keywords: ["appearance", "theme", resolvedTheme, option.id],
        codeThemeId: option.id,
        variant: resolvedTheme,
        isActive: activeTheme.codeThemeId === option.id,
      })),
    [activeTheme.codeThemeId, messages.searchPalette, resolvedTheme],
  );
  ...
  const importFieldLabel = messages.searchPalette.sessionIdLabel;
  const importPlaceholder = messages.searchPalette.sessionIdPlaceholder;
  ...
  const browseSubmitLabel = willCreateMissingFolder
    ? messages.searchPalette.createAndAdd
    : messages.searchPalette.add;
  ...
  const submitBrowsePath = async () => {
    ...
    if (trimmedQuery.length === 0 && !highlightedFolderPath) {
      setAddProjectError(messages.searchPalette.errorEnterFolderPath);
      return;
    }
    if (unsupportedWindowsPath) {
      setAddProjectError(messages.searchPalette.errorWindowsPath);
      return;
    }
    if (!highlightedFolderPath && isExplicitRelativeProjectPath(trimmedQuery)) {
      setAddProjectError(messages.searchPalette.errorRelativePath);
      return;
    }
    ...
    try {
      ...
      setAddProjectError(
        cause instanceof Error ? cause.message : messages.searchPalette.errorAddProjectFallback,
      );
    } finally {
      setIsAddingProject(false);
    }
  ...
  const submitImport = async () => {
    ...
    try {
      ...
      setImportError(error instanceof Error ? error.message : messages.searchPalette.importFailed);
    } finally {
      setIsImporting(false);
    }
  };
            ...
            <div className="border-b border-border/70 px-4 py-3">
              <div className="flex items-start gap-3">
                <Button
                  size="icon"
                  variant="ghost"
                  aria-label={messages.searchPalette.backAria}
                ...
                <div>
                  <p className="text-sm font-medium text-foreground">
                    {messages.searchPalette.importHeading}
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {messages.searchPalette.importDescription}
                  </p>
                </div>
              </div>
            </div>
            <div className="space-y-4 px-4 py-4">
              <div className="space-y-2">
                <p className="text-xs font-medium uppercase tracking-[0.08em] text-muted-foreground">
                  {messages.searchPalette.providerLabel}
                </p>
                ...
                {props.importProviders.length === 0 ? (
                  <p className="text-xs text-muted-foreground">
                    {messages.searchPalette.noImportProviders}
                  </p>
                ) : null}
              </div>
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  {messages.searchPalette.sessionIdHelp}
                </p>
              </div>
              {importError ? (
              ...
              <div className="flex justify-end gap-2">
                ...
                >
                  {isImporting
                    ? messages.searchPalette.importing
                    : messages.searchPalette.importAction}
                </Button>
              </div>
            </div>
          </div>
              ...
              <CommandPanel className="overflow-hidden">
                <div className="relative">
                  <CommandInput
                    placeholder={
                      isBrowsing
                        ? messages.searchPalette.browsePlaceholder
                        : messages.searchPalette.searchPlaceholder
                    }
                    value={query}
                  ...
                  {isBrowsing ? (
                    <Button
                      ...
                      title={
                        hasHighlightedFolderItem
                          ? messages.searchPalette.addHighlightedFolder(
                              browseSubmitLabel,
                              submitModifierLabel,
                            )
                          : messages.searchPalette.addWithEnter(browseSubmitLabel)
                      }
                    >
                ...
                <CommandList className="max-h-[min(24rem,60vh)] not-empty:px-1.5 not-empty:pt-0 not-empty:pb-1.5">
                    ...
                    unsupportedWindowsPath ? (
                      <CommandEmpty className="py-10">
                        <div className="text-center text-sm text-muted-foreground/79">
                          {messages.searchPalette.errorWindowsPath}
                        </div>
                      </CommandEmpty>
                    ) : (
                      <>
                        {canBrowseUp || filteredBrowseEntries.length > 0 ? (
                          ...
                          <div className="px-3 py-2 text-sm text-muted-foreground">
                            {messages.searchPalette.noMatchingFolders}
                          </div>
                        ) : null}
                        {willCreateMissingFolder ? (
                          <div className="mx-1.5 mt-2 rounded-md border border-dashed border-[color:var(--color-border)] px-3 py-2 text-sm text-muted-foreground">
                            {messages.searchPalette.createFolderHintPrefix}{" "}
                            <span className="text-foreground">{trimmedQuery}</span>{" "}
                            {messages.searchPalette.createFolderHintSuffix}
                          </div>
                        ) : null}
                        {addProjectError ? (
                  ...
                  {!isBrowsing && matchedActions.length > 0 ? (
                    <CommandGroup>
                      <CommandGroupLabel className="pt-0 pb-1.5 pl-3">
                        {messages.searchPalette.suggestedGroup}
                      </CommandGroupLabel>
                            ...
                            <div className="min-w-0 flex-1">
                              <div className="flex items-baseline gap-3">
                                <div className="min-w-0 flex-1 truncate text-[length:var(--app-font-size-ui,12px)] text-foreground">
                                  <HighlightedText
                                    text={thread.title || messages.searchPalette.untitledThread}
                                    query={query}
                                  />
                                </div>
                  ...
                  {!isBrowsing && matchedProjects.length > 0 ? (
                    <CommandGroup>
                      <CommandGroupLabel className="py-1.5 pl-3">
                        {messages.searchPalette.projectsGroup}
                      </CommandGroupLabel>
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="truncate text-[length:var(--app-font-size-ui,12px)] text-foreground">
                              {project.name || messages.searchPalette.untitledProject}
                            </div>
                  ...
                  {showThemeSection ? (
                    <>
                      {themeCommandItem ? (
                        <CommandGroup>
                          <CommandGroupLabel className="py-1.5 pl-3">
                            {messages.searchPalette.configureGroup}
                          </CommandGroupLabel>
                          <CommandItem
                      ...
                      {matchedCurrentThemes.length > 0 ? (
                        <CommandGroup>
                          <CommandGroupLabel className="py-1.5 pl-3">
                            {resolvedTheme === "dark"
                              ? messages.searchPalette.darkThemesGroup
                              : messages.searchPalette.lightThemesGroup}
                          </CommandGroupLabel>
                            ...
                            return (
                              ...
                              >
                                ...
                                <span className="shrink-0 text-[length:var(--app-font-size-ui-meta,10px)] text-muted-foreground/79">
                                  {resolvedTheme === "dark"
                                    ? messages.searchPalette.darkColorTheme
                                    : messages.searchPalette.lightColorTheme}
                                </span>
                                <span
                  ...
                  {!isBrowsing && !hasSearchResults ? (
                    <CommandEmpty className="py-10">
                      <div className="flex flex-col items-center justify-center gap-2 text-center text-sm text-muted-foreground/79">
                        ...
                        <div>{messages.searchPalette.noMatches}</div>
                      </div>
                    </CommandEmpty>
                  ) : null}
                </CommandList>
              ...
              <CommandFooter>
                {isBrowsing ? (
                  <>
                    <span>
                      {isAddingProject
                        ? messages.searchPalette.addingProject
                        : messages.searchPalette.browseHint}
                    </span>
                    <span>
                      {hasHighlightedFolderItem
                        ? messages.searchPalette.enterToOpenWithModifier(submitModifierLabel)
                        : hasHighlightedBrowseItem
                          ? messages.searchPalette.enterToGoUp
                          : messages.searchPalette.enterToAddProject}
                    </span>
                  </>
                ) : (
                  <>
                    <span>{messages.searchPalette.inputHint}</span>
                    <span>{messages.searchPalette.enterHint}</span>
                  </>
                )}
              </CommandFooter>
            </Command>
