...
import { RotatingFileSink } from "@peakcode/shared/logging";
...
const browserPerfLoggingEnabled =
  process.env.PEAKCODE_BROWSER_PERF === "1" || process.env.PEAKCODE_BROWSER_PERF === "1";
...
function startBrowserPerformanceLogging(): void {
  ...
  browserPerfInterval = setInterval(() => {
    ...
    console.info(`[${PEAKCODE_BROWSER_LABEL} perf]`, {
      ...snapshot.counters,
...
async function reserveBackendEndpoint(reason: string): Promise<void> {
  ...
  process.env.PEAKCODE_DESKTOP_WS_URL = backendWsUrl;
  process.env.PEAKCODE_DESKTOP_WS_URL = backendWsUrl;
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { peakcodeCommitHash?: unknown };
    return normalizeCommitHash(parsed.peakcodeCommitHash);
  } catch {
    return null;
  }
}
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.PEAKCODE_COMMIT_HASH);
  if (envCommitHash) {
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("Peak Code failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function handleCheckForUpdatesMenuClick(): void {
  const hasUpdateFeedConfig =
    readAppUpdateYml() !== null || Boolean(process.env.PEAKCODE_DESKTOP_MOCK_UPDATES);
  const disabledReason = getAutoUpdateDisabledReason({
    isDevelopment,
    isPackaged: app.isPackaged,
    platform: process.platform,
    appImage: process.env.APPIMAGE,
    disabledByEnv: process.env.PEAKCODE_DISABLE_AUTO_UPDATE === "1",
    hasUpdateFeedConfig,
  });
  if (disabledReason) {
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `Peak Code ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
...
function resolveNotificationIconPath(): string | null {
  ...
  if (process.platform === "win32") {
    return resolveResourcePath("peakcode.png") ?? resolveIconPath("ico");
  }
  return resolveResourcePath("peakcode.png") ?? resolveIconPath("png");
}
...
function resolveUserDataPath(): string {
  ...
  if (seedResult.status === "seeded") {
    console.info("[desktop] Seeded Peak Code Electron profile from legacy profile", {
      sourcePath: seedResult.sourcePath,
      targetPath: seedResult.targetPath,
    });
  } else if (seedResult.status === "seed-failed") {
    console.warn("[desktop] Failed to seed Peak Code Electron profile from legacy profile", {
      sourcePath: seedResult.sourcePath,
      targetPath: seedResult.targetPath,
...
function configureAutoUpdater(): void {
  ...
  const githubToken =
    process.env.PEAKCODE_DESKTOP_UPDATE_GITHUB_TOKEN?.trim() || process.env.GH_TOKEN?.trim() || "";
...
function backendEnv(): NodeJS.ProcessEnv {
  return {
    ...process.env,
    PEAKCODE_MODE: "desktop",
    PEAKCODE_NO_BROWSER: "1",
    PEAKCODE_PORT: String(backendPort),
    PEAKCODE_HOME: BASE_DIR,
    PEAKCODE_AUTH_TOKEN: backendAuthToken,
    [PEAKCODE_BROWSER_USE_PIPE_ENV]: PEAKCODE_BROWSER_USE_PIPE_PATH,
  };
}
