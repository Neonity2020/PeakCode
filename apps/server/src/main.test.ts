...
const runCli = (
  ...
  env: Record<string, string> = {
    PEAKCODE_HOME: "/tmp/t3-test-home",
...
it.layer(testLayer)("server CLI command", (it) => {
  ...
  it.effect("uses env fallbacks when flags are not provided", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        ...
        PEAKCODE_PORT: "4999",
        PEAKCODE_HOST: "100.88.10.4",
        PEAKCODE_HOME: "/tmp/t3-env-home",
        VITE_DEV_SERVER_URL: "http://localhost:5173",
        PEAKCODE_NO_BROWSER: "true",
        PEAKCODE_AUTH_TOKEN: "env-token",
      });
  ...
  it.effect("prefers --mode over PEAKCODE_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over PEAKCODE_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        PEAKCODE_NO_BROWSER: "false",
      });
  ...
  it.effect("supports CLI and env for bootstrap/provider-log/websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        PEAKCODE_MODE: "desktop",
        PEAKCODE_LOG_PROVIDER_EVENTS: "true",
        PEAKCODE_LOG_WS_EVENTS: "false",
        PEAKCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        PEAKCODE_NO_BROWSER: "true",
      });
