...
beforeEach(() => {
  ...
  registry.getApiKeyAndHeaders.mockResolvedValue({ ok: true, apiKey: "secret", headers: { "x-test": "yes" } });
  vi.mocked(completeSimple).mockResolvedValue({ stopReason: "stop" } as Awaited<ReturnType<typeof completeSimple>>);
});
afterEach(() => vi.useRealTimers());

describe("testModelProvider", () => {
  it("uses the saved model and resolved auth in the selected directory", async () => {
    expect(await testModelProvider({ agentDir: "/custom-agent", provider: "custom", modelId: "model" }))
    ...
    expect(completeSimple).toHaveBeenCalledWith(model, expect.objectContaining({ messages: expect.any(Array) }),
      expect.objectContaining({ apiKey: "secret", headers: { "x-test": "yes" }, maxTokens: 64, maxRetries: 0 }));
  });

  it("falls back to the first provider model and default directory", async () => {
    expect(await testModelProvider({ provider: "custom" })).toEqual({ status: "success", model: "custom/model" });
  ...
  it("does not expose upstream errors or credentials", async () => {
    ...
    vi.mocked(completeSimple).mockResolvedValue({ stopReason: "error", errorMessage: "secret" } as Awaited<ReturnType<typeof completeSimple>>);
    expect(await testModelProvider({ provider: "custom" })).toEqual({ status: "request-failed", model: "custom/model" });
  });
  ...
  it("does not start a request if auth resolves after the deadline", async () => {
    ...
    registry.getApiKeyAndHeaders.mockReturnValue(new Promise((resolve) => { resolveAuth = resolve; }));
    const result = testModelProvider({ provider: "custom" });
    await vi.advanceTimersByTimeAsync(20_000);
    expect(await result).toEqual({ status: "timeout" });
