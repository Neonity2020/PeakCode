...
import { ApprovalRequestId, ThreadId } from "@peakcode/contracts";
...
describe("buildCodexProcessEnv", () => {
  ...
  it("allows the configured desktop browser-use socket in the Codex sandbox", () => {
    const env = buildCodexProcessEnv({
      env: {
        PEAKCODE_BROWSER_USE_PIPE_PATH: "/tmp/codex-browser-use/peakcode.sock",
    ...
    expect(env.NODE_REPL_SANDBOX_ALLOWED_UNIX_SOCKETS).toBe(
      "/tmp/existing.sock,/tmp/codex-browser-use/peakcode.sock",
    );
  });
  ...
  it("resolves the browser-use pipe path from desktop env aliases", () => {
    expect(
      resolveCodexBrowserUsePipePath({
        env: { PEAKCODE_BROWSER_USE_PIPE_PATH_LEGACY: "/tmp/codex-browser-use/t3.sock" },
        platform: "darwin",
      }),
    ).toBe("/tmp/codex-browser-use/t3.sock");
  });

  it("disables the local peakcode-browser plugin in Peak Code's Codex home overlay", () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "t3-codex-env-"));
  ...
  it("adds a disabled peakcode-browser plugin section when Codex config does not contain one", () => {
...
describe("startSession", () => {
  it("enables Codex experimental api capabilities during initialize", () => {
    expect(buildCodexInitializeParams()).toEqual({
      clientInfo: {
        name: "peakcode_desktop",
        title: "Peak Code Desktop",
        version: "0.1.0",
      },
      capabilities: {
  ...
  it("uses an isolated scratch workspace path when no cwd is provided", () => {
    ...
    expect(cwd).toContain(`${path.sep}peakcode-codex-workspaces${path.sep}thread-1`);
  });
  ...
  it("fails fast with an upgrade message when codex is below the minimum supported version", async () => {
    ...
    const versionCheck = vi
      ...
      .mockImplementation(() => {
        throw new Error(
          "Codex CLI v0.36.0 is too old for Peak Code. Upgrade to v0.37.0 or newer and restart Peak Code.",
        );
      });
