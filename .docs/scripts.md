...
- Dev commands default `PEAKCODE_HOME` to `~/.peakcode`, which keeps dev state under `~/.peakcode/dev`.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --home-dir ~/.peakcode-2`
...
- `bun run release:smoke` — Replays the release-only steps (version alignment, lockfile refresh, macOS updater manifest merge) against a temporary copy of every workspace manifest.
...
- Desktop packaging includes `apps/server/dist` (the `peakcode` backend) and starts it on loopback with an auth token for WebSocket/API traffic.
...
Set `PEAKCODE_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `PEAKCODE_DEV_INSTANCE`)
- Example: `PEAKCODE_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `PEAKCODE_PORT_OFFSET` to a numeric offset.
