...
export function parsePatch(patch: string): PatchOp[] {
  ...
  if (lines[lines.length - 1]!.trim() !== END_PATCH_MARKER) {
    throw new PatchError(`Invalid patch: The last line of the patch must be '${END_PATCH_MARKER}'`);
  }
    ...
    if (line.startsWith(UPDATE_FILE_MARKER)) {
      ...
      while (index < end) {
        ...
        if (trimmed === "@@" || trimmed.startsWith("@@ ")) {
          // 先绑到局部常量再推：`ensureHunk` 也是给 `current` 赋值的闭包，
          // 调用它之后 TS 会放弃对 `current` 的收窄，直接 push 过不了类型检查。
          const opened: PatchHunk = {
            lines: [],
            context: trimmed === "@@" ? undefined : trimmed.slice(3).trim(),
          };
          current = opened;
          hunks.push(opened);
          index += 1;
  ...
  try {
    for (const item of pending) {
      ...
      if (write) {
        ...
        undo.push(() =>
          previous === null ? fs.remove(write.path) : fs.write(write.path, previous),
        );
      }
