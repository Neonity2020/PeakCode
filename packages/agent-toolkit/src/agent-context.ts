...
import { getHistory } from "./chat.ts";
 ...
 * 与工作区有关，只有宿主拿得到），估算路径下会被算进去。
 *
 * `windowTokens` 可以覆盖：原实现把窗口当成全局设置（同一台机器跑同一个本地模型），
 * 而按模型区分窗口的宿主（比如 PeakCode 的 pi 会话）需要按会话传实际模型窗口进来 ——
 * 否则占用条和真正触发压缩的阈值会对不上。
 */
export function contextUsage(
  ...
  const windowTokens = opts.windowTokens ?? contextWindowTokens();
