...
export function skillsPromptSection(options: SkillsPromptOptions = {}): string | null {
  ...
  return [
    ...
    skills.length > shown.length
      ? `（还有 ${skills.length - shown.length} 个技能未列出，需要时可用 read_skill 直接按名字读取。）`
      : "",
  ]
    .filter((line) => line !== "")
    .join("\n");
...
export function readSkillFile(name: string, file?: string): SkillFileResult {
  ...
  try {
    ...
    return {
      ok: false,
      reason: `读不到技能目录：${error instanceof Error ? error.message : String(error)}`,
    };
  }
  ...
  if (!existsSync(target))
    return { ok: false, reason: `技能 ${name} 里没有这个文件：${file ?? "SKILL.md"}` };
  try {
    if (!statSync(target).isFile())
      return { ok: false, reason: `${file ?? "SKILL.md"} 不是一个文件` };
    ...
    if (!TEXT_EXTENSIONS.has(extension)) {
      return {
        ok: false,
        reason: `不读二进制 / 未知类型的文件（${extension || "无后缀"}），技能里通常不需要它`,
      };
    }
