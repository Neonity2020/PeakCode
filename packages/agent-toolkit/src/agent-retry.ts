...
import { isRetryableAssistantError } from "./pi-compat.ts";
 ...
 */
/**
 * `attachTurnRecovery` 需要的最小宿主形状。
 *
 * `shouldStopAfterTurn` 挂在哪儿是 pi 的版本差异：0.85 的 `Agent` 直接带这个可写属性，
 * 0.74 里同一个钩子走 `AgentLoopConfig.shouldStopAfterTurn`（`Agent` 不暴露）。
 * 用结构类型描述需求，宿主按自己的版本接线，这个模块不必跟着 pi 的大版本走。
 */
...
export interface TurnRecoveryOptions {
  /** 本回合已经走到第几步（步数到顶时优先停，不再多跑一轮）。 */
  steps: () => number;
  maxSteps: number;
  /** 最多提醒几次（设置 `AGENT_RETRY_MAX`-同源；0 = 关掉空回合自愈）。 */
  budget: number;
  onNudge: (attempt: number) => void;
}

/**
 * 造一个 `shouldStopAfterTurn` 钩子：回合结束时的自愈判定。
 *
 * 返回钩子而不是"直接挂上去"，是因为 pi 0.74 的 `Agent` 没有这个可写属性 ——
 * 0.85 上挂实例属性，0.74 上把返回值传进 loop config。只挂不返回的话，
 * 在 0.74 上会**静默失效**：空回合不再提醒，而代码看起来一切正常。
 */
export function createTurnRecovery(
  host: TurnRecoveryHost,
  opts: TurnRecoveryOptions,
): (context: ShouldStopAfterTurnContext) => boolean {
  let nudges = 0;
  return (context) => {
    ...
    nudges += 1;
    opts.onNudge(nudges);
    host.followUp(nudgeMessage(nudges));
    return false;
  };
}

/** 便捷形式：把自愈钩子挂到支持实例属性的宿主上（pi ≥ 0.85）。返回该钩子。 */
export function attachTurnRecovery(
  host: TurnRecoveryHost,
  opts: TurnRecoveryOptions,
): (context: ShouldStopAfterTurnContext) => boolean {
  const hook = createTurnRecovery(host, opts);
  host.shouldStopAfterTurn = hook;
  return hook;
}
