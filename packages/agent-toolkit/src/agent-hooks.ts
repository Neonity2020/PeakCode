...
export function parseHookConfigs(raw: string): { hooks: HookConfig[]; errors: string[] } {
  ...
  try {
    ...
    return {
      hooks: [],
      errors: [
        `AGENT_HOOKS 不是合法 JSON：${error instanceof Error ? error.message : String(error)}`,
      ],
    };
  }
...
export async function runHook(
  ...
  try {
    proc = spawnProcess([shell, "-c", hook.command], {
      env: {
        ...process.env,
        PEAKCODE_HOOK_EVENT: hook.event,
        PEAKCODE_HOOK_PAYLOAD: serialized,
      },
      cwd:
    ...
    return {
      blocked: false,
      error: `启动失败：${error instanceof Error ? error.message : String(error)}`,
    };
  }
