...
export function recordArtifact(input: {
  ...
  const existing = store.findArtifactByPath(input.conversationId, abs);

  if (existing) {
    store.updateArtifact(existing.id, {
      size,
      createdAt: Date.now(),
      tool: input.tool,
  ...
  if (item)
    for (const cb of listeners) cb({ conversationId: input.conversationId, artifact: item });
  return item;
}

function toItem(row: AgentArtifactRecord): ArtifactItem {
  return {
    id: row.id,
