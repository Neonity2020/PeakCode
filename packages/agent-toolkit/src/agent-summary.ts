...
import { type CompleteSimpleLike, serializeConversation } from "./pi-compat.ts";
import { recordTokenUsage } from "./runtime/usage.ts";
import { estimateMessagesTokens, type MessageLike } from "./token-estimate.ts";
...
export async function summarizeHistory<T extends { role?: string; content?: unknown }>(input: {
  ...
  const transcript = serializeConversation(region);
