...
import { globDirectories, spawnSyncCapture } from "./runtime/spawn.ts";
  ...
  const proc = spawnSyncCapture(
    ...
    ],
    {
      env: {
        ...
        GIT_AUTHOR_NAME: "PeakCode Agent",
        GIT_AUTHOR_EMAIL: "agent@peakcode.local",
        GIT_COMMITTER_NAME: "PeakCode Agent",
        GIT_COMMITTER_EMAIL: "agent@peakcode.local",
        ...(opts.env ?? {}),
      },
...
export function gitAvailable(): boolean {
  try {
    return spawnSyncCapture(["git", "--version"]).code === 0;
  } catch {
    return false;
  }
}
...
export function ensureSnapshotRepo(
  workspace: string,
): { ok: true; repoDir: string } | { ok: false; error: string } {
...
export function createTurnSnapshot(input: {
  ...
  const turnNumber =
    index.records.filter((record) => record.conversationId === input.conversationId).length + 1;
...
export function listTurnSnapshots(conversationId?: number): TurnSnapshot[] {
  ...
  for (const entry of globDirectories(base, "index.json")) {
...
export function previewSnapshotChanges(id: string): {
  ...
  const diff = runShadowGit(repoDir, workspace, ["diff", "--name-status", id], {
    allowFailure: true,
  });
...
export function revertToSnapshot(id: string): RevertResult {
  ...
  const reset = runShadowGit(repoDir, workspace, ["read-tree", "--reset", "-u", id], {
    allowFailure: true,
  });
  if (!reset.ok) {
...
export function snapshotGcThresholds(): { bytes: number; turns: number } {
  ...
  const turns =
    Number.isFinite(rawTurns) && rawTurns > 0 ? Math.max(10, Math.floor(rawTurns)) : MAX_RECORDS;
  return { bytes, turns };
}
 ...
 */
export function maybeGcSnapshotRepo(
  workspace: string,
  opts: { force?: boolean } = {},
): SnapshotGcResult {
  ...
  if (!before) {
    return {
      ran: false,
      reason: "还没有影子仓库（这个工作区没跑过 Agent 回合）",
      bytesBefore: 0,
      bytesAfter: 0,
      freedBytes: 0,
    };
