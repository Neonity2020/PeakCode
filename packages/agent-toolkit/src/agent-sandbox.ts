...
import { landlockCommand, landlockHelper, probeLandlockWorkspace } from "./landlock-helper.ts";
import { getDataDir, isAgentDataPath } from "./runtime/paths.ts";
import { spawnSyncCapture } from "./runtime/spawn.ts";
...
export function effectiveSandboxBackend(
  ...
  opts: {
    bwrapReady?: boolean | undefined;
    landlockReady?: boolean | undefined;
    prefer?: SandboxBackendPreference | undefined;
  } = {},
): SandboxBackend {
...
export function bwrapAvailable(opts: { refresh?: boolean } = {}): boolean {
  ...
  try {
    ...
    const probe = spawnSyncCapture(["bwrap", "--ro-bind", "/", "/", "--dev", "/dev", "true"]);
    bwrapProbe = probe.code === 0;
  } catch {
    bwrapProbe = false;
  }
  if (bwrapProbe === false && sandboxBackend() === "bwrap") {
    logEvent({
      ...
      message:
        "本机没有可用的 bubblewrap（未安装，或容器里不允许非特权 user namespace），Linux 沙箱降级为不沙箱",
...
export function sandboxSupported(
  platform: string = process.platform,
  opts: { bwrapReady?: boolean | undefined; landlockReady?: boolean | undefined } = {},
): boolean {
...
export function sandboxCredentialPaths(home = process.env.HOME ?? "/"): string[] {
  ...
  if (isAgentDataPath(identifierDir)) paths.push(identifierDir);
  return paths;
}
...
export function sandboxProfile(opts: {
  ...
  const writable = subpathFilters(
    sandboxWritableRoots(opts.workspace, opts.authorizedFolders ?? []),
  );
...
export function landlockUnavailableReason(workspace: string, canaryReady?: boolean): string | null {
  ...
  if (!sandboxAllowsNetwork()) {
    return (
      "Landlock 后端不支持禁网（内核 net 规则未实现），已让位给 bubblewrap；" +
      "想只用 Landlock 就把沙箱的「允许联网」打开"
    );
  }
  const canary =
    canaryReady === undefined ? probeLandlockWorkspace(workspace) : { ok: canaryReady };
  if (!canary.ok) {
 ...
 */
export function explainSandboxDenial(
  output: string,
  opts: { backend?: SandboxBackend } = {},
): string | null {
