...
beforeEach(() => {
  dir = mkdtempSync(path.join(tmpdir(), "peakcode-hooks-"));
...
describe("单条 hook 的契约", () => {
  test("事件 JSON 从 stdin 交给脚本；事件名在环境变量里", async () => {
    ...
    const fromEnv = await runHook(hook(`printf '%s' "$PEAKCODE_HOOK_EVENT"`), {
      event: "user_prompt_submit",
      conversationId: 7,
      workspace: dir,
      mode: "agent",
  ...
  test("只有明确的 block 才拦回合；失败与超时都只是警告", async () => {
    ...
    const slow = await runHook(
      { event: "user_prompt_submit", command: "sleep 5", timeoutMs: 200 },
      {
        event: "user_prompt_submit",
        conversationId: 1,
        workspace: dir,
