...
describe("补丁应用", () => {
  test("多文件改动一次落盘，摘要与 Codex 同款 A/M/D", () => {
    ...
    expect(result.summary).toBe(
      ["Success. Updated the following files:", "A src/new.ts", "M src/old.ts", "D gone.txt"].join(
        "\n",
      ),
    );
  ...
  test("重命名 + 改写；目标已存在时报错且不动原文", () => {
    ...
    const blocked = apply(patch("*** Update File: a.txt\n*** Move to: b.txt\n@@\n-old\n+new\n"));
  ...
  test("追加到文件末尾（*** End of File）不会插到开头", () => {
    ...
    const result = apply(patch("*** Update File: log.txt\n@@\n second\n+third\n*** End of File"));
    if (!result.ok) throw new Error(result.error);
  ...
  test("路径解析交给调用方：越界时抛错，整体不落盘", () => {
    ...
    const result = applyPatch(patch("*** Update File: /etc/passwd\n@@\n-ok\n+hacked\n"), {
      resolve: () => {
