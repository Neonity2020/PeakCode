...
describe("影子仓库", () => {
  ...
  test("工作区里已有的 git 仓库不受影响", () => {
    ...
    const userRepo = spawnSyncCapture(["git", "init", "--quiet", workspace]);
    expect(userRepo.code).toBe(0);
    write("a.txt", "v1\n");
    ...
    const status = spawnSyncCapture(["git", "-C", workspace, "status", "--porcelain"]);
