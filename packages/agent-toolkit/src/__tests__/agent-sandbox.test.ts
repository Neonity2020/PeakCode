...
import { landlockHelperAvailable } from "../landlock-helper.ts";
...
describe("策略与包装", () => {
  ...
  test("阻断说明只在真的像沙箱拦截时出现，并说清是哪一档", () => {
    updateSettings({ AGENT_SANDBOX_MODE: "workspace-write" });
    expect(explainSandboxDenial("zsh:1: operation not permitted: /Users/x/a.txt")).toContain(
      "工作区 / 临时目录",
    );
    updateSettings({ AGENT_SANDBOX_MODE: "read-only" });
    expect(explainSandboxDenial("zsh:1: operation not permitted: /Users/x/a.txt")).toContain(
...
describe("Linux（bwrap）", () => {
  ...
  test("拦截解释认得 bwrap 的报错（只读挂载 / bwrap 前缀）", () => {
    ...
    expect(explainSandboxDenial("bwrap: Can't create file at /tmp/x: Permission denied")).toContain(
      "沙箱",
    );
...
describe("Linux（Landlock 规则生成）", () => {
  ...
  test("凭据清单里的本应用数据目录是**真实路径**（写死名字等于什么都没拦）", () => {
    ...
    // 不能再有那个匹配不到任何东西的占位条目。
    expect(creds.some((p) => p.endsWith(path.join("Application Support", "omni-studio")))).toBe(
      false,
    );
  });
  ...
  test("后端偏好：默认 auto（bwrap 优先、Landlock 兜底），可显式指定", () => {
    ...
    expect(effectiveSandboxBackend("linux", { bwrapReady: true, landlockReady: true })).toBe(
      "bwrap",
    );
    expect(effectiveSandboxBackend("linux", { bwrapReady: false, landlockReady: true })).toBe(
      "landlock",
    );
    // 都没有：仍报首选后端（bwrap），由 supported=false + 降级说明交代，不静默换后端。
    expect(effectiveSandboxBackend("linux", { bwrapReady: false, landlockReady: false })).toBe(
      "bwrap",
    );
    // 显式偏好：想用 Landlock 就用，但它不可用时也不会假装。
    expect(
      effectiveSandboxBackend("linux", {
        bwrapReady: true,
        landlockReady: true,
        prefer: "landlock",
      }),
...
describe("端到端（真实 Seatbelt）", () => {
  test("工作区内可写，工作区外不可写", async () => {
    ...
    const inside = await bash.execute("s1", {
      command: "echo inside-ok > note.txt && cat note.txt",
    });
  ...
  test("read-only：工作区也写不了、临时目录可以写、读照常", async () => {
    ...
    try {
      ...
      const tmpWrite = await bash.execute("ro4", {
        command: `echo scratch > ${tmpTarget} && cat ${tmpTarget}`,
      });
      ...
      try {
        ...
        const denied = await bash.execute("ro5", {
          command: `echo nope > ${path.join(authorized, "x.txt")}`,
        });
  ...
  test("沙箱升级：被拦后按次申请，允许则跳过沙箱重跑一次", async () => {
    ...
    try {
      ...
      const bash = tools.find((tool) => tool.name === "bash")!;
      const result = await bash.execute("esc-allow", {
