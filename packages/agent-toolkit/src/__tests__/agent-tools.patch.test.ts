...
import { permissionRequestForTool } from "../permissions.ts";
...
describe("view_image 工具", () => {
  ...
  test("视觉模型才有这个工具；纯文本模型拿不到", () => {
    expect(buildReadOnlyTools({ ...ctx, vision: false }).some((t) => t.name === "view_image")).toBe(
      false,
    );
    expect(buildReadOnlyTools({ ...ctx, vision: true }).some((t) => t.name === "view_image")).toBe(
      true,
    );
  });
...
describe("request_permissions（模型主动申请权限）", () => {
  test("工具能跑起来就意味着闸门已放行；工作区内不必多问一次", async () => {
    ...
    const granted = await tool!.execute("p1", {
  ...
  test("补丁里只要有一个文件在工作区外，就按 external_directory 询问", () => {
    const request = permissionRequestForTool({
      ...
      args: {
        patch: wrap(
          [
            "*** Update File: inside.md",
            "@@",
            "-a",
            "+b",
            "*** Add File: /tmp/evil.sh",
            "+rm -rf /",
          ].join("\n"),
        ),
      },
