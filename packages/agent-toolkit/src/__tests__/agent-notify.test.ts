...
import { externalNotifyConfigured, notifyCommand, notifyExternal } from "../agent-notify.ts";
...
beforeEach(() => {
  dir = mkdtempSync(path.join(tmpdir(), "peakcode-notify-"));
...
describe("外部通知回调", () => {
  ...
  test("命令写错也只是发不出去，不影响调用方", () => {
    ...
    expect(() => notifyExternal({ kind: "info", title: "照常入通知中心" })).not.toThrow();
    expect(typeof notifyExternal({ kind: "info", title: "同样返回 false 或 true" })).toBe(
      "boolean",
    );
  });
});
