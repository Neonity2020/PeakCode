...
import type { AgentGoalRecord } from "./store/records.ts";
...
function toGoal(row: AgentGoalRecord): AgentGoal {
  return {
...
export function getGoal(conversationId: number): AgentGoal | null {
  const row = agentStore().getGoal(conversationId);
...
export function clearGoal(conversationId: number): void {
  agentStore().clearGoal(conversationId);
...
export function goalPromptSection(conversationId: number): string | null {
  ...
  return [
    ...
    goal.acceptance
      ...
      : "**验收标准**：（尚未明确 —— 先用 ask_user 与用户对齐，再写进 goal 工具）",
