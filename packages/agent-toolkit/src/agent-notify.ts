 ...
 * `PEAKCODE_NOTIFY_PAYLOAD`（shell 命令串里直接 `"$PEAKCODE_NOTIFY_PAYLOAD"` 更好写）。
 * 两者都不做字符串拼接 —— 标题里带引号或分号也只是数据，不会被当命令执行。
 *
 * 触发点只有一个：宿主通知中心的每一条（回合跑完 / 需要授权 / 自动化结果 / 出错）
 * 都调一次 `notifyExternal`，不重复埋点。
 *
 * 任何失败都只写统一日志：外部命令是用户自己的东西，不能反过来影响 Agent 运行。
 */
import { logEvent } from "./runtime/log.ts";
import { getSetting } from "./runtime/settings.ts";
import { spawnProcess } from "./runtime/spawn.ts";
 ...
 */
export function notifyExternal(input: {
  kind: string;
  title: string;
  body?: string;
  conversationId?: number | null;
  ...
  try {
    ...
    const proc = spawnProcess([shell, "-c", `${command} "$1"`, "peakcode-notify", serialized], {
      env: { ...process.env, PEAKCODE_NOTIFY_PAYLOAD: serialized },
      detached: true,
    });
    proc.unref();
    return true;
